	...
	list_for_each_entry(rsvd_pkt, &rtwdev->rsvd_page_list, list) {
		if (rsvd_pkt->type == RSVD_PROBE_REQ) {
			if (!ssid && !rsvd_pkt->ssid)
				location = rsvd_pkt->page;
			else if (rtw_ssid_equal(rsvd_pkt->ssid, ssid))
				location = rsvd_pkt->page;
		}
	}
...
static struct sk_buff *rtw_nlo_info_get(struct ieee80211_hw *hw)
{
	...
	memset(nlo_hdr->pattern_check, 0xA5A5A5A5, 4);
					   ...
					   struct ieee80211_vif *vif)
{
	...
	conf->pattern_cam_backup = !!rtw_wow->pattern_cnt;
...
void rtw_fw_channel_switch(struct rtw_dev *rtwdev, bool enable)
{
	...
	u8 h2c_pkt[H2C_PKT_SIZE] = {0};
	u16 total_size = H2C_PKT_HDR_SIZE + 20;
	u8 loc_ch_info;
	struct rtw_ch_switch_option cs_option = {
		.dest_ch_en = 1,
		.dest_ch = 1,
		.periodic_option = 2, /*PERIODIC_2_PHASE*/
		.normal_period = 5,
		.normal_period_sel = 0,
		.normal_cycle = 10,
		.slow_period = 1,
...
void rtw_fw_config_rsvd_page(struct rtw_dev *rtwdev)
{
	...
	switch (rtw_wow->suspend_mode) {
	case RTW_SUSPEND_NO_LINK:
		...
		break;
	case RTW_SUSPEND_IDLE:
	case RTW_SUSPEND_LINKED:
	default:
