...
static int create_sdw_dailink(struct device *dev, int *link_index,
			      struct snd_soc_dai_link *dai_links,
			      int sdw_be_num, int sdw_cpu_dai_num,
			      ...
			      bool *ignore_pch_dmic)
{
	...
	for_each_pcm_streams(stream) {
		...
		ret = set_codec_init_func(link, dai_links + (*link_index)++,
					  playback, group_id);
		if (ret < 0) {
				     ...
				     struct snd_soc_card *card)
{
	...
	for (; adr_link->num_adr; adr_link++) {
		...
		ret = create_sdw_dailink(dev, &link_index, links, sdw_be_num,
...
SSP:
	...
	for (i = 0, j = 0; ssp_mask; i++, ssp_mask >>= 1) {
		...
		ret = info->init(NULL, links + link_index, info, 0);
		if (ret < 0)
			return ret;
