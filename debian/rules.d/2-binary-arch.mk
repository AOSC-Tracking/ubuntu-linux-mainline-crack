...
$(stampdir)/stamp-install-%: dbgpkgdir_dkms = $(if $(filter true,$(do_dbgsym_package)),$(dbgpkgdir)/usr/lib/debug/usr/lib/modules/$(abi_release)-$*/kernel,"")
...
ifeq ($(no_dumpfile),)
	...
	$(kmake) O=$(build_dir) $(conc_level) modules_install $(vdso) \
		INSTALL_MOD_STRIP=1 INSTALL_MOD_PATH=$(pkgdir)/usr
	...
	done >>$(pkgdir)/usr/lib/modprobe.d/blacklist_$(DEB_SOURCE)_$(abi_release)-$*.conf
...
ifeq ($(no_dumpfile),)
	...
	rm -f $(pkgdir)/usr/lib/modules/$(abi_release)-$*/build
	rm -f $(pkgdir)/usr/lib/modules/$(abi_release)-$*/source

	# Some initramfs-tools specific modules
	install -d $(pkgdir)/usr/lib/modules/$(abi_release)-$*/initrd
	if [ -f $(pkgdir)/usr/lib/modules/$(abi_release)-$*/kernel/drivers/video/vesafb.ko ]; then\
	  $(LN) $(pkgdir)/usr/lib/modules/$(abi_release)-$*/kernel/drivers/video/vesafb.ko \
		$(pkgdir)/usr/lib/modules/$(abi_release)-$*/initrd/; \
	fi
...
ifeq ($(do_dbgsym_package),true)
	...
	rm -f $(dbgpkgdir)/usr/lib/debug/usr/lib/modules/$(abi_release)-$*/build
	rm -f $(dbgpkgdir)/usr/lib/debug/usr/lib/modules/$(abi_release)-$*/source
	rm -f $(dbgpkgdir)/usr/lib/debug/usr/lib/modules/$(abi_release)-$*/modules.*
...
ifeq ($(build_arch),s390)
	...
	# The build symlink
	install -d debian/$(basepkg)-$*/usr/lib/modules/$(abi_release)-$*
	$(LN) /usr/src/$(basepkg)-$* \
		debian/$(basepkg)-$*/usr/lib/modules/$(abi_release)-$*/build
	...
	#
	mkdir $(pkgdir)/usr/lib/modules/$(abi_release)-$*/_
	mv $(pkgdir)/usr/lib/modules/$(abi_release)-$*/modules.order \
		$(pkgdir)/usr/lib/modules/$(abi_release)-$*/_
	if [ -f $(pkgdir)/usr/lib/modules/$(abi_release)-$*/modules.builtin ] ; then \
	    mv $(pkgdir)/usr/lib/modules/$(abi_release)-$*/modules.builtin \
		$(pkgdir)/usr/lib/modules/$(abi_release)-$*/_; \
	fi
	if [ -f $(pkgdir)/usr/lib/modules/$(abi_release)-$*/modules.builtin.modinfo ] ; then \
	    mv $(pkgdir)/usr/lib/modules/$(abi_release)-$*/modules.builtin.modinfo \
		$(pkgdir)/usr/lib/modules/$(abi_release)-$*/_; \
	fi
	rm -f $(pkgdir)/usr/lib/modules/$(abi_release)-$*/modules.*
	mv $(pkgdir)/usr/lib/modules/$(abi_release)-$*/_/* \
		$(pkgdir)/usr/lib/modules/$(abi_release)-$*
	rmdir $(pkgdir)/usr/lib/modules/$(abi_release)-$*/_
...
ifeq ($(do_cloud_tools),true)
 ifeq ($(do_tools_hyperv),true)
	...
	$(foreach _m,$(all_dkms_modules), \
	  $(if $(enable_$(_m)), \
	    $(call build_dkms,$(dkms_$(_m)_pkg_name)-$*,$(dkms_$(_m)_pkgdir)/usr/lib/modules/$(abi_release)-$*/$(dkms_$(_m)_subdir),$(dbgpkgdir_dkms),$(_m),$(dkms_$(_m)_debpath)); \
	  ) \
	)
...
ifeq ($(do_dbgsym_package),true)
	...
	find $(pkgdir) \
	  -name '*.ko' | while read path_module ; do \
		module="/usr/lib/modules/$${path_module#*/usr/lib/modules/}"; \
	...
	if [ -f $(pkgdir)/usr/lib/modules/$(abi_release)-$*/modules.builtin ] ; then \
		sed -e 's/.*\/\([^\/]*\)\.ko/\1/' $(pkgdir)/usr/lib/modules/$(abi_release)-$*/modules.builtin | \
	...
	if [ -f $(pkgdir)/usr/lib/modules/$(abi_release)-$*/modules.builtin.modinfo ] ; then \
		cat $(pkgdir)/usr/lib/modules/$(abi_release)-$*/modules.builtin.modinfo | \
...
ifeq ($(do_tools_perf),true)
	cd $(builddirpa)/tools/perf && \
		LLVM_CONFIG=llvm-config-21 $(kmake) prefix=/usr HAVE_CPLUS_DEMANGLE_SUPPORT=1 CROSS_COMPILE=$(CROSS_COMPILE) NO_LIBPERL=1 WERROR=0
endif
ifeq ($(do_tools_bpftool),true)
